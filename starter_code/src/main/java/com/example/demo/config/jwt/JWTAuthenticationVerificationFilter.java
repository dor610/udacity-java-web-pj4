...
public class JWTAuthenticationVerificationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest req) {
        String token = req.getHeader(Constant.Security.HEADER_STRING);
        if (token != null) {
            String user = JWT.require(Algorithm.HMAC512(Constant.Security.SECRET.getBytes())).build()
                    .verify(token.replace(Constant.Security.TOKEN_PREFIX, ""))
                    .getSubject();
            if (user != null) {
